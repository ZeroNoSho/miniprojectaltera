...
export default function Home() {
  const { transaksiklr, col, token } = useContext(Contex);
  ...
  const deletss = async (e) => {
    const res = await axios.delete(`/api/transaksikeluar/delet/${e}`, {
      headers: {
  ...
  const multiDimensionalArray =
    transaksiklr &&
    transaksiklr?.transaksikeluar.map((obj) => {
      return {
        No_Struk: obj.no_struk,
        Tanggal: obj.createAt.replace(/T/gi, " | ").replace("000Z", " "),
        Nama: obj.databarang[0].nama_barang,
        Tipe: "keluar",
        Satuan: obj.databarang[0].satuan,
        Jumlah: obj.jumlah,
        Harga: obj.harga,
      };
    });
